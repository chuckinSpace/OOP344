...
Item::Item(const std::string& record){
    ...
    if(more){temp_serial = myTempObj.extractToken(record,next_pos,more);}
    
    if(more){temp_quantity = myTempObj.extractToken(record,next_pos,more);}
    updateWidth(temp_quantity.size());
    
    if(more){m_description = myTempObj.extractToken(record,next_pos,more);}
